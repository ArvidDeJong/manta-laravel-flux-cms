...
<flux:table.cell align="right">
    <flux:dropdown align="end" offset="-15">
        ...
        <flux:menu class="min-w-32">
            @if ($item->trashed())
                <flux:modal.trigger name="member-restore{{ $item->id }}">
                    <flux:menu.item icon="arrow-path" variant="danger">Herstellen
                    </flux:menu.item>
                </flux:modal.trigger>
            @else
                @php
                ...
                @if (!isset($item->locked) || !$item->locked)
                    <flux:modal.trigger name="member-remove{{ $item->id }}">
                        <flux:menu.item icon="trash" variant="danger">Verwijderen
                        </flux:menu.item>
                    </flux:modal.trigger>
                @endif
    ...
    <flux:modal name="member-remove{{ $item->id }}" class="min-w-[22rem]">
        <form class="space-y-6" wire:submit="deleteConfirm({{ $item->id }})">
            <div>
    ...
    <flux:modal name="member-restore{{ $item->id }}" class="min-w-[22rem]">
        <form class="space-y-6" wire:submit="restoreConfirm({{ $item->id }})">
            <div>
