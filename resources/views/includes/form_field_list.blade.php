                 ...
                 <div x-data="{
                     ...
                     <div class="flex flex-wrap gap-2 rounded-md border p-2">
                         <template x-for="tag in tags" :key="tag">
                             <span class="inline-flex items-center rounded bg-blue-100 px-2 py-1 text-sm text-blue-800">
     ...
     @if (count($data_fields) > 0)
         ...
         @foreach ($data_fields as $key => $datafield)
             @if ($datafield['type'] == 'string')
                 <flux:input wire:model.live="data_content.{{ $key }}" label="{{ $datafield['title'] }}" />
