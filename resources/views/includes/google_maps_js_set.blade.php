<script>
    ...
    // Livewire: luister naar wijzigingen in GOOGLE_MAPS_ZOOM
    if (window.Livewire) {
        window.Livewire.on('refreshGoogleMapsZoom', function(zoom) {
    ...
    function createMarker(id, latitude, longitude, title, draggable = true) {
        ...
        markers[id].addListener('dragend', function() {
            ...
            @this.set('DEFAULT_LATITUDE', position.lat());
            @this.set('DEFAULT_LONGITUDE', position.lng());
